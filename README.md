...
- A **discord-send sample plugin** is scaffolded the same way: thumbnail grid picker, trim UI, one-click render to 10/25/50/100MB via bundled ffmpeg, then drag-and-drop straight into Discord (real OS drag) or Open folder. It also adds a **Send to Discord** entry to every clip's ⋯ menu. Shared loader APIs (`api.clipPath`, `api.thumbUrl`, `api.ClipGrid`, `api.registerClipAction`) power both samples and are available to your own plugins.
